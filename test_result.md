...
frontend:
  ...
  - task: "Stair Cube Game (مكعب الدرج) - 5th game"
    ...
    status_history:
      ...
      - working: true
        agent: "main"
        comment: "Major visual + physics overhaul (V3): (a) STAIRS made MUCH wider — STAIR_W=TILE*5 (140px), STAIR_H=TILE*1.5 (42px), 3.3:1 ratio. (b) TRUE 3D CUBE via react-native-svg: 8 vertices rotated through XYZ matrices, perspective-projected (persp=8 for orthographic look), all 6 faces drawn with z-sorting (no back-face culling so cube is never open). Lambert shading per-face. The cube now rotates around all 3 axes during flight. (c) installed react-native-svg@15.12.1 (matching Expo SDK version) — earlier 15.15.4 was broken. (d) Default isometric tilt rotX=0.55 rotY=-0.65 so cube shows top+front+right faces at rest. (e) PHYSICS overhaul: realistic restitution (BOUNCE_DAMP=0.34), low ground friction (0.35 → lots of sliding), proper SIDE WALL collision detection on BOTH left and right walls (cube now bounces off stair vertical edges in both directions), realistic angular impulse based on impact speed/type — top hits cause forward tumble proportional to vx, side hits cause Y-axis spin. (f) Viewport bucket state forces visibleStairs to recompute as cube moves so stairs appear instantly during fast flight. (g) Cube SVG now auto-aligns visual bottom with math bottom (yOffset = halfSize - maxProjectedY) so the cube SITS PERFECTLY on the stair top — no more floating above or sinking below. (h) Stair design improved: highlight strip on top edge, gradient on front face, proper edge colors per stair type (blue=untouched, green=touched, gold=power)."
