...
frontend:
  - task: "Arabic Wordle Game - Full gameplay, map, keyboard, storage"
    ...
    status_history:
      ...
      - working: true
        agent: "main"
        comment: "Second polish iteration: (a) Greatly expanded dictionary to 640 words total (285 4-letter + 240 5-letter + 115 6-letter), separating ANSWER pool (used as level targets) from EXTENDED dictionary (accepted as guesses). Now common words like تموز, العاب, خميس, جمعة, تفاح, جواب, طعام, شراب, ابيض, اسود, اخضر, etc. are all accepted. (b) Made level word fully RANDOM via pickRandomWordForLevel(level, avoidPrev): on first entry to a level a fresh random word is picked from the pool; on retry from loss modal, a NEW different word (not equal to the previous one) is picked, satisfying the user's request for 'different word each restart'. Verified with 2 retries showing 2 different target words (لعبة → اقرب)."
